...
export default function Product({ id }: Readonly<InferGetStaticPropsType<typeof getStaticProps>>) {
  ...
  return (
    <>
      <Head>
        <title>Rocket Front | Produto { id }</title>
        <meta name="description" content="Pagina do produto" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      { isLoading ? <div>Loading...</div> : <main className="flex container mx-auto mt-8">
        <Image
        ...
        <section className="flex flex-col justify-between ml-8">
          ...
          <button type="button" className="flex items-center justify-center bg-[#115D8C] hover:bg-[#5C5ADB] transition ease-in-out text-white py-3"
            onClick={() => addItemToCart({ ...data!, qtd: 1, price: 40 })}
          >
